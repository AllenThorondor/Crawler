...
class Teller:
    ...
    def db_openup(self, db_name, table_name):
        """
...
# In[24]:
def process(web_count, conn, cur):
    count = 0
    retrieved_list = list()
    ill_list = list()
    while True:
        retrieved_list = cur.execute('''SELECT url FROM URLs WHERE label = ?''',(RETRIEVED_LINK,))
        ill_list = cur.execute('''SELECT url FROM URLs WHERE label = ?''',(ILL_LINK,))
